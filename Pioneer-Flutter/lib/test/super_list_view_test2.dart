...
class SuperListViewTest2 extends StatefulWidget {
  SuperListViewTest2({Key key}) : super(key: key);
...
class _SuperListViewTestState extends State<SuperListViewTest2> {
  ...
  void initState() {
    ...
    _pageLoad = PageLoad2<String>(
        ...
        statusController: _statusController);
    getData(_pageLoad.getPage(false), _pageLoad).then((value) {
  ...
  Widget build(BuildContext context) {
    return SuperListView2(
      ...
      onItemReload: () {
        getData(_pageLoad.getPage(true), _pageLoad).then((value) {
          _pageLoad.loadData(value);
        });
      },
      onLoadMore: () {
